 ...
 * 2003/09/28 | ŠeŽíƒoƒOƒtƒBƒNƒX•‘½­‚ÌÅ“K‰» ( by H.Kawai )
 *
 */
...
typedef struct
{
    ...
    int scan_ac[3];
    int scan_dc[3];
    int scan_h[3];  // ƒTƒ“ƒvƒŠƒ“ƒO—v‘f”
    int scan_v[3];  // ƒTƒ“ƒvƒŠƒ“ƒO—v‘f”
    int scan_qt[3]; // —ÊŽq‰»ƒe[ƒuƒ‹ƒCƒ“ƒfƒNƒX

    // DRI
    int interval;

    int mcu_buf[32*32*4]; //ƒoƒbƒtƒ@
    int *mcu_yuv[4];
    int mcu_preDC[3];
	...
	int base_img[64][64]; // Šî’ê‰æ‘œ ( [‰¡Žü”g”uƒÎ][cŽü”g”vƒÎ][‰¡ˆÊ‘Š(M/8)][cˆÊ‘Š(N/8)]
...
//	if (jpeg->width == 0)
//		return 8;
	/* decode0‚Å‚Íinfo‚µ‚Ä‚©‚çŒÄ‚Î‚ê‚é‚Ì‚ÅA‚±‚ê‚Í‚È‚¢ */
...
unsigned short get_bits(JPEG *jpeg, int bit)
{
	...
	while (remain <= 16) {
		...
		c = *jpeg->fp++;
		if (c == 0xff) { // ƒ}[ƒJƒGƒ‰[‚ð–h‚®‚½‚ßAFF -> FF 00 ‚ÉƒGƒXƒP[ƒv‚³‚ê‚Ä‚é
			if (jpeg->fp >= jpeg->fp1) {
				ret = 0;
				goto fin;
			}
	...
	while (size > 0) {
		...
		if (j > jpeg->n_dqt)
 		...
 		if (c & 0xf8) {
			// 16 bit DQT
			for (i = 0; i < 64; i++) {
				jpeg->dqt[j][i] = jpeg->fp[0]; /* ‰ºˆÊ‚Í“Ç‚ÝŽÌ‚Ä‚Ä‚¢‚é */
				jpeg->fp += 2;
			}
			size += -64 * 2;
		} else {
...
int jpeg_dht(JPEG *jpeg)
{
	...
	while (len > 0) {
		...
		tc = (val >> 4) & 0x0f; // ƒe[ƒuƒ‹ƒNƒ‰ƒX(DC/AC¬•ªƒZƒŒƒNƒ^)
		th =  val       & 0x0f; // ƒe[ƒuƒ‹ƒwƒbƒ_(‰½–‡–Ú‚ÌƒvƒŒ[ƒ“‚©)
...
int jpeg_decode_mcu(JPEG *jpeg, UCHAR *zigzag_table)
{
	...
	// mcu_width x mcu_heightƒTƒCƒY‚ÌƒuƒƒbƒN‚ð“WŠJ
